...
export default (testContext: {
  ...
  describe('DID manager', () => {
    ...
    it.only('should change owner of alice DID to bobs key', async () => {
      const result = await agent.didManagerUpdateControllerKey({
        did: alice.did,
        kid: bob.keys[0].kid
      })
      expect(result).toContain('0x')
      const updatedAlice = await agent.didManagerGet({ did: alice.did })
      expect(updatedAlice.keys[0].kid).toEqual(bob.keys[0].kid)
    })
  })
}
